<style>
  ...
  .toggle-switch:hover[disabled='false'] {
    background-color: #ff6c94;
  }
  .toggle-switch[toggled='true'] {
    background-color: #30fde7;
  }
  :not(.disabled) .toggle-switch[toggled='true']:hover {
    background-color: #d8fffa;
  }

  :not(.disabled) .toggle-label:active .toggle-switch {
...
<script>
  export let id = `toggle-${Math.random()}`;
...
<div class="toggle" class:disabled>
  <label class="toggle-label">
    ...
    <span class="toggle-track">
      <span class="toggle-switch" {disabled} {toggled} />
    </span>
    {#if onLabel && offLabel}
      <label for={id} class="text-label">{toggled ? onLabel : offLabel}</label>
    {/if}
  </label>
</div>
