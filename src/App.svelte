<script>
  ...
  import {createRandomCoordinate, isValidMove, nextHeadCoordinate, willEat, willCollide} from './utils/game-board';
  ...
  const nextFood = state => willEat(state) ? createRandomCoordinate(state.gridSize) : state.foodCoordinate;
  ...
  const enqueue = (state, move) => isValidMove(move, state) ? merge(state)({moves: [...state.moves, move]}) : state;
